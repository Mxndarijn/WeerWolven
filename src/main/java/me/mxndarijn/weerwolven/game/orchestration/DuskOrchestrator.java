...
import me.mxndarijn.weerwolven.game.orchestration.executor.AbilityExecutorHelper;
...
public final class DuskOrchestrator extends PhaseOrchestrator {
    ...
    public void runCollection(Runnable onDone) {
        ...
        // Use AbilityExecutorHelper to handle the go-home logic
        String timerId = "dusk:home:" + game.getDayNumber();
        AbilityExecutorHelper.goHome(alive, game, timerId).thenAccept(success -> {
            for (GamePlayer gp : game.getGamePlayers()) {
                game.getGameHouseManager().setCanOpenDoor(gp, false);
                game.getGameHouseManager().clearOnPlayerReturnHome(gp);
            }
            // Close all house doors for provided players
            game.getGameHouseManager().closeAllDoors(game.getGamePlayers());
            game.getGameHouseManager().closeAllHouseWindows(game.getGamePlayers());
            onDone.run();
        });
    }
